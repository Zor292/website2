...
// ── TRUST PROXY (مهم جداً على Railway) ─────────────────────
...
app.use(session({
  ...
  cookie: {
    secure: true,        // Railway دائماً HTTPS
    httpOnly: true,
    sameSite: 'lax',
    maxAge: 86400000 * 7 // أسبوع
  }
}));
...
app.get('/auth/discord', (req, res) => {
  console.log('Redirecting to Discord OAuth, REDIRECT_URI:', CONFIG.REDIRECT_URI);
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    // 3. Save session and WAIT for it to be saved
    ...
    console.log(`✅ Login success: ${userRes.data.username} | session id: ${req.session.id}`);
...
app.get('/api/staff', async (req, res) => {
  try {
    const rolesRes = await axios.get(`${DISCORD_API}/guilds/${GUILD_ID}/roles`,
      { headers: { Authorization: `Bot ${CONFIG.BOT_TOKEN}` } });
    ...
    const membersRes = await axios.get(`${DISCORD_API}/guilds/${GUILD_ID}/members?limit=100`,
      { headers: { Authorization: `Bot ${CONFIG.BOT_TOKEN}` } });
...
function requireAuth(req, res, next) {
  console.log('requireAuth - session user:', req.session.user?.username || 'none');
